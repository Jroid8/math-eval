...
use crate::number::{BfPointer, BuiltinFunc, ExtraFuncId, Number};
...
pub enum FunctionType<E: ExtraFuncId, C: FuncId> {
    Builtin(BuiltinFunc<E>),
    Custom(C),
}

impl<E: ExtraFuncId + Display, C: FuncId + Display> Display for FunctionType<E, C> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            FunctionType::Builtin(bf) => <BuiltinFunc<E> as Display>::fmt(bf, f),
...
pub enum AstNode<N: Number, V: VarId, F: FuncId> {
    ...
    BinaryOp(BinaryOp),
    UnaryOp(UnaryOp),
    Function(FunctionType<N::ExtraFuncId, F>, NonZeroU8),
}
...
impl<N: Number, V: VarId, F: FuncId> Node for AstNode<N, V, F> {
    fn children(&self) -> usize {
        match self {
            ...
            AstNode::UnaryOp(_) => 1,
            AstNode::Function(_, argc) => argc.get(),
        }
        .into()
    }
}
...
pub struct CfInfo<F: FuncId> {
    pub ident: F,
    pub min_args: NonZeroU8,
    pub max_args: Option<NonZeroU8>,
}

impl<F: FuncId> CfInfo<F> {
    pub const fn new(ident: F, min_args: NonZeroU8, max_args: Option<NonZeroU8>) -> Self {
        CfInfo {
            ident,
            min_args,
            max_args,
    ...
    fn eval_stack_capacity<'a>(tree: impl IntoIterator<Item = &'a AstNode<N, V, F>>) -> usize {
        ...
        for node in tree {
            stack_len += match node {
                ...
                AstNode::UnaryOp(_) => 0,
                AstNode::Function(_, args) => -(args.get() as i64) + 1,
            };
    ...
    fn _eval<'a, 'b>(
        ...
        for (idx, node) in tree.into_iter().enumerate() {
            ...
            let result: N = match node {
                ...
                AstNode::Function(FunctionType::Builtin(bf), argc) => match bf.get_method_ptr() {
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        for (node, edge, idx) in self.0.euler_tour() {
            ...
            if let Some((AstNode::Function(_, argc), p)) =
                ...
                && self.0.nth_child(p, argc.get() as usize - 1).unwrap() != idx
                && edge == NodeEdge::End
            {
                f.write_str(", ")?;
            }
...
mod tests {
    ...
    impl TestFunc {
        const fn with_info(self) -> CfInfo<Self> {
            match self {
                TestFunc::Deg2Rad => CfInfo::new(TestFunc::Deg2Rad, nz!(1), Some(nz!(1))),
                TestFunc::ExpD => CfInfo::new(TestFunc::ExpD, nz!(2), Some(nz!(2))),
                TestFunc::Clamp => CfInfo::new(TestFunc::Clamp, nz!(3), Some(nz!(3))),
    ...
    fn parse_to_ast() {
        ...
        assert_eq!(
            ...
            Ok(vec![
                AstNode::Number(14.0),
                AstNode::<f64, TestVar, TestFunc>::Function(StdFloatFunc::Sin.into(), nz!(1)),
            ])
        );
        assert_eq!(
            ...
            Ok(vec![
                AstNode::Number(80.0),
                AstNode::Function(FunctionType::Custom(TestFunc::Deg2Rad), nz!(1)),
            ])
        );
        assert_eq!(
            ...
            Ok(vec![
                AstNode::Number(0.2),
                AstNode::Variable(TestVar::X),
                AstNode::Function(FunctionType::Custom(TestFunc::ExpD), nz!(2)),
            ])
        );
        assert_eq!(
            ...
            Ok(vec![
                ...
                AstNode::Variable(TestVar::Y),
                AstNode::Variable(TestVar::X),
                AstNode::Function(FunctionType::Custom(TestFunc::Clamp), nz!(3)),
            ])
        );
        assert_eq!(
            ...
            Ok(vec![
                ...
                AstNode::Number(2.0),
                AstNode::Variable(TestVar::X),
                AstNode::Function(FunctionType::Custom(TestFunc::Digits), nz!(6)),
            ])
        );
        assert_eq!(
            syntaxify("lb(8)"),
        ...
        assert_eq!(
            ...
            Ok(vec![
                ...
                AstNode::Function(StdFloatFunc::Ln.into(), nz!(1)),
            ])
        );
        assert_eq!(
            syntaxify("sin(cos(0))"),
            Ok(vec![
                AstNode::Number(0.0),
                AstNode::Function(StdFloatFunc::Cos.into(), nz!(1)),
                AstNode::Function(StdFloatFunc::Sin.into(), nz!(1)),
            ])
        );
        assert_eq!(
            syntaxify("x^2 + sin(y)"),
        ...
        assert_eq!(
            ...
            Ok(vec![
                AstNode::Number(4.0),
                AstNode::Number(9.0),
                AstNode::Function(BasicFunc::Max.into(), nz!(2)),
        ...
        assert_eq!(
            ...
            Ok(vec![
                ...
                AstNode::Number(1.0),
                AstNode::BinaryOp(BinaryOp::Add),
                AstNode::Function(BasicFunc::Max.into(), nz!(4)),
            ])
        );
        assert_eq!(
            syntaxify("2*x + 3*y"),
        ...
        assert_eq!(
            ...
            Ok(vec![
                ...
                AstNode::Function(BasicFunc::Log10.into(), nz!(1)),
            ])
        );
        assert_eq!(
            syntaxify("1/(2+3)"),
    ...
    fn aot_evaluation() {
        ...
        assert_eq!(
            simplify(&[
                ...
                AstNode::Number(0.0),
                AstNode::Function(StdFloatFunc::Sin.into(), nz!(1)),
                AstNode::Function(BasicFunc::Min.into(), nz!(2)),
                AstNode::Function(BasicFunc::Max.into(), nz!(3)),
    ...
    fn displacing_simplification() {
        ...
        assert_eq!(
            simplify(&[
                AstNode::Number(17.0),
                AstNode::Variable(TestVar::Y),
                AstNode::Function(BasicFunc::Log.into(), nz!(1)),
                AstNode::BinaryOp(BinaryOp::Sub),
                AstNode::Number(10.0),
                AstNode::Number(1.0),
                AstNode::BinaryOp(BinaryOp::Sub),
